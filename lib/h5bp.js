...
wikiAnchors = function wikiAnchors(text, config) {
  ...
  text = text.replace(/\[\[([^|\]]+)\|([^\]]+)\]\]/g, function(wholeMatch, m1, m2) {
    var ext = /\/\//.test(m2),
    path = ext ? m2 : Path.join(bu, m2.split(' ').join('-') + "/index.html");
    return "["+m1+"](" + path + ")";
  });
...
server = function server(config) {
  ...
  var s = connect.createServer()
    .use(connect.logger({format: '> :date :method :status :url'}))
    .use(config.serveUrl || '', connect.static(Path.join(config.dest)))
    .listen(config.port);
...
copyAssets = function copyAssets(config) {
  var src = config.assetsDir ? config.assetsDir : Path.resolve(__dirname, '..', 'public'),
  to = Path.resolve(config.dest, 'public');

  return copyDir(src, to, function(err) {
    if(err) throw err;
    if (config.assetsFiles === false) return;
    config.assetsFiles.forEach(function(file) {
      exec('cp ' + file + ' ' + Path.join(to, Path.basename(file)), function(err) {
        if (err) throw err;
      });
    });
  });
  ...
  ensureDir(destPath, function process() {
    ...
    findit(srcPath)
      .on('file', function(file) {
        ...
        // also takes care of files beginning with `.` like `.htaccess`
        var filename = file.replace(srcPath + '/', '').replace(fileReg, '').replace(/^\./, ''),
        title = filename.replace(/-/g, ' '),
        href = title === 'Home' ? '/index.html' : Path.join('', destPath, Path.basename(filename), '/index.html').replace(destPath, '');
      ...
      .on('end', function() {
        ...
        files.forEach(function(file) {
          ...
          ensureDir(dest, function() {
            ...
            if((fileCount--) === 1) {
              // undefined assetsDir -> copy of local public folder
              // false assetsDir -> prevent the copy
              // any other value will copy over the assetsDir folder.
              if(config.assetsDir !== false) copyAssets(config);
              server(config);
            }
          });
